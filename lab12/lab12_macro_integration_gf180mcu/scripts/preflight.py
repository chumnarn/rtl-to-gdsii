...
def main() -> int:
    ...
    pdn = top_cfg.get("PDN_MACRO_CONNECTIONS", [])
    expected = "u_counter_macro VDD VSS VDD VSS"
    if expected not in pdn:
        errors.append(
            "top/config.yaml must contain PDN connection: "
            + expected
        )
