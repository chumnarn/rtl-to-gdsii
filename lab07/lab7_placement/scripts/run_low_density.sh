...
TMP_CONFIG="$(mktemp "${PROJECT_DIR}/.lab7-low.XXXXXX.yaml")"
trap 'rm -f -- "${TMP_CONFIG}"' EXIT

cd "${PROJECT_DIR}"
python3 - config.yaml "${TMP_CONFIG}" 35 50 <<'PY'
import re, sys
...
PY
librelane --run-tag lab7_low_density --to OpenROAD.DetailedPlacement "${TMP_CONFIG}"
